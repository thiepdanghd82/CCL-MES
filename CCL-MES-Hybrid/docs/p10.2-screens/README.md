...
## P10.2 LOGIN→GRID END-TO-END VERIFIED (2026-06-03, clean build)

`07-grid-clean-build.png` shows the NPI Work Centers grid rendered inside the Mac Catalyst MAUI Blazor Hybrid app, after the `engineer / engineer` login completed against the embedded `CCL.MES.Api` running on `localhost:5100` against the seeded SQLite DB at `<repo>/data/ccl_mes.db` (43 active Work Centers). `08-home-clean-build.png` shows the home page (`Xin chào, Demo Engineer` + NPI tile + sidebar nav + sign-out button) that the user lands on immediately after the JWT pair is stored. Both screenshots are from a fully-cleaned production-shape build:
- All DEEP-DEBUG probe panel + `AUTO LOGIN` button removed from `Login.razor`.
- All boot-trace + `[Login] HandleSubmitAsync entered` / `[TokenStore] SaveAsync ...` Console.WriteLine traces removed from prod code paths.
- `WKWebView.Inspectable = true` + the `cclLog` WKScriptMessageHandler bridge + `window.onerror` observer are gated on `#if DEBUG && (MACCATALYST || IOS)` — Release builds get neither inspectability nor any JS log forwarding.
- The Tab+Enter Catalyst keyboard-trap workaround was extracted into `Shared/MacCatalystKeyboardFix.razor`, a platform-gated component that detects Catalyst at runtime via UA (`Mobile/` AND `Mac OS X`) and is injected into both `MainLayout` and `EmptyLayout` so every page benefits. iOS / Windows / Android builds render the component as a no-op (UA check fails). Removing the workaround once Apple ships their fix is one line per layout.
- `MauiSecureTokenStore` falls back to an in-memory dictionary FOR THIS PROCESS when Keychain returns `MissingEntitlement` — the case on every adhoc-signed dev build, since no `keychain-access-groups` entitlement is in scope. Signed production builds will use Keychain normally. Without the fallback, every login throws and the user sees "Đăng nhập thất bại" — with it, login succeeds + tokens stay valid for the app's lifetime.
- `appsettings.json` `CclApi:BaseUrl` switched from `http://localhost:5100` to `http://127.0.0.1:5100` because the Mac Catalyst HttpClient's IPv6 preference for `localhost` reaches `::1:5100` first and the macOS networking stack returns `Connection refused` faster than the IPv6→IPv4 fallback completes. Pinning IPv4 explicitly closes that latency window — same DB, same JWT, same flow.

`log-11-login-flow-success.txt` is the full ~20-line app + TokenStore + Login + NavigateTo trace from the version that still had the diagnostic Console.WriteLine entries (before the final cleanup pass). Reviewers can scan it to see the exact event sequence: `[Login] HandleSubmitAsync entered. u='engineer' p.len=8` → `[Login] calling Api.LoginAsync...` → `[Login] Api.LoginAsync returned access.len=597.` → `[TokenStore] SaveAsync caught Exception: Error adding record: MissingEntitlement. IsEntitlementFailure=True` → `[TokenStore] Keychain unavailable ... Falling back to in-memory token store` → `[Login] Session.SetLoginAsync returned. Navigating to target.` → `[Login] NavigateTo(/) returned.`
