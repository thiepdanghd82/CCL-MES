...
| `gate-qc-process-kind.sh` chưa có `--self-test` | Đo 2026-09-16: `gate-all --self-test` ra `SKIP=1` vì gate này. Mục "3 gate cũ chưa có self-test" ở trên đã đóng năm 08-23 với lời hứa `SKIP=0`; gate qc-process-kind thêm vào 09-11 lại không kèm self-test, nên lời hứa đó đã âm thầm hỏng. Nợ nhỏ, nhưng để nguyên thì con số SKIP mất ý nghĩa cảnh báo. |
| `IqcDocumentViewerTests.Zoom_va_xoay_doi_trang_thai_hien_thi` chớp tắt ~1/6 | Đo 2026-09-16, và đo CẢ HAI phía để khỏi đổ oan: cây có thay đổi 1 đỏ/10 lượt, cây SẠCH (stash toàn bộ) 1 đỏ/6 lượt ⇒ **có sẵn, không do đợt nhãn phase**. Chạy RIÊNG thì 5/5 xanh nên nghi tương tranh khi chạy song song cả bộ. Chưa RCA — và theo bài học cũ của chính backlog này, đừng gắn nhãn "flake" rồi nới assert; phải đo phân bố rồi tìm nguyên nhân thật. |
