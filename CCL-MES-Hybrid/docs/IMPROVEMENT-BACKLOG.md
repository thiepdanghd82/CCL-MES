...
| `WoIpqcMaterialChecks` mang chữ ký nhưng FK là `CASCADE` — **lệch luật thống nhất 16-09**, đo 22-09-2026 | Luật đã chốt: hồ sơ QC **có chữ ký** → `RESTRICT` · dữ liệu **thao tác** → `CASCADE`. Quét toàn DB tìm bảng có cột chữ ký (`ApprovedBy`/`ConfirmedBy`/`SignedBy`/`ApprovalReason`/…) rồi đối chiếu `pragma_foreign_key_list`: **3 bảng có cột chữ ký mà vẫn `CASCADE` về `WorkOrders`** — `WoIpqcMaterialChecks` (**4 dòng**) · `QcInspections` (0) · `WoSettingCheckItems` (0). `WoQcChecks` thì đúng `RESTRICT`. **Bảng đáng lo là bảng đầu:** 4 dòng đều của WO-TEST-02 (id 59), và **3/4 mang chữ ký 4 mắt thật** (`ConfirmedBy=admin` → `ApprovedBy=qc`, `DivergenceApprovalStatus=Approved`) ⇒ xoá WO là xoá im lặng ba phê duyệt sai lệch vật tư. **Không có lưới đỡ nào từ phiếu cha:** bảng chỉ có **đúng một** FK (về `WorkOrders`); cột `WoIpqcCheckId` **không có FK** và **NULL ở cả 4 dòng**, nên `RESTRICT` của `WoIpqcChecks` không chắn hộ. **Vì sao gate 29 không bắt:** `EVIDENCE="WoTraceSnapshot WoIpqcCheck WoQcCheck"` là danh sách **viết tay** ở đầu `gate-wo-child-fk.sh` — gate canh đúng luật nhưng chỉ trên 3 bảng người viết nhớ ra. **Chưa được tự sửa ngay:** có cột `ApprovedBy` **không đương nhiên** là hồ sơ ký thật — hai bảng 0 dòng kia chưa ai xác minh là phiếu có chữ ký hay chỉ là cột thừa, và đổi `CASCADE`→`RESTRICT` là đổi hành vi xoá trên production. Việc phải làm, theo thứ tự: ① xác minh từng bảng có phải hồ sơ ký không ② bảng nào đúng thì vào `EVIDENCE` + migration A→B→C ③ cân nhắc đổi gate từ danh sách viết tay sang **suy ra từ cột chữ ký** để không sót lần sau. Liên quan: cổng **D3** trong [tờ trình STOP-gate](https://claude.ai/artifact/PEFeT6oMDHea4uLwUfZJnY) (cùng bảng, câu hỏi khác: cổng `/run/start` có công nhận duyệt IPQC không). |
