...
| **Cơ chế chặn tái phát** | Gate `scripts/gate-no-hardcoded-hex.sh` (đã test: PASS cây hiện tại; FAIL khi thêm hex thô ngoài khối `:root` trong `app.css`) — màu mới PHẢI qua token. Rule trong `:root` (comment `RULE: no raw hex (L37)`) + SKILLS.md S15. Invariant grep `^\s*--[a-z0-9-]+\s*:\s*var\(--\1\)` = 0 (không định nghĩa token vòng). **Quy tắc: đổi/thêm màu = sửa/ thêm TOKEN, KHÔNG hardcode hex trong `app.css`; retone = swap giá trị token, không find/replace hex.** |
