#!/usr/bin/env bash
# P10.6b end-to-end verify script. Pattern locked in P10.6a.
#
# Usage (always from repo root):
#   cd CCL-MES-Hybrid && ./scripts/verify-p10.6b.sh
#   cd CCL-MES-Hybrid && ./scripts/verify-p10.6b.sh --keep-alive
#   cd CCL-MES-Hybrid && ./scripts/verify-p10.6b.sh --verbose
#
# Scope:
#   P10.6b ships zero new server endpoints — Appearance is a
#   client-only language picker page (/settings/appearance) backed by
#   MAUI Preferences via ILanguageService. The verify script
#   therefore focuses on (1) the new client lib code paths via unit
#   tests, (2) the existing server still boots + serves the Settings
#   surface clean (no regression), and (3) the canary routes still
#   401 anon so the renderer-dead lesson + P10.6a/d/f route discovery
#   guards stay green.
...
cleanup() {
    if [[ $KEEP_ALIVE -eq 1 && $FAIL -eq 0 ]]; then
        ...
        echo "             REBUILD the Catalyst client app (Razor changes — picker page),"
        echo "             relaunch, log in, then verify the 7-step Catalyst checklist:"
        echo "             1. Sidebar CÀI ĐẶT shows new 'Giao diện' sub-link."
        echo "             2. Settings landing → 'Giao diện' group card visible (1 card)."
        echo "             3. /settings/appearance loads — 2 language cards (VN active by default)."
        echo "             4. Tap 'English' card → border + check ✓ flip; warning banner appears"
        echo "                ('Bản dịch tiếng Anh sẽ áp dụng sau khi cập nhật ứng dụng.')."
        echo "             5. Tap 'Tiếng Việt' → flips back; banner gone."
        echo "             6. Theme section shows 3 disabled cards (sáng/tối/system) marked sắp có."
        echo "             7. Quit + relaunch app → selection survives (Preferences persisted)."
...
echo "===================================================================="
echo "P10.6b verify — $(date '+%Y-%m-%d %H:%M:%S')"
echo "===================================================================="
...
# ── 2. Run client unit tests (incl. new Localization coverage) ───────
...
# ── 2b. Confirm new Localization coverage exists + passes ────────────
echo "[step] filter-run Localization tests to prove coverage"
LOC_OUT=$(cd "$REPO_ROOT" && dotnet test "$CLIENT_TESTS" -c Debug --nologo --no-build \
    --filter "FullyQualifiedName~Localization" 2>&1)
LOC_RC=$?
LOC_PASSED=$(echo "$LOC_OUT" | grep -oE 'Passed:[[:space:]]+[0-9]+' | head -1 | grep -oE '[0-9]+')
[[ -z "$LOC_PASSED" ]] && LOC_PASSED=0
[[ $VERBOSE -eq 1 ]] && echo "$LOC_OUT" | tail -6
# 21 expected: 6 in InMemoryLanguageServiceTests + 15 in LanguageCodeNamesTests
# (4 theories with multiple InlineData rows expand to more).
if [[ $LOC_RC -eq 0 && $LOC_PASSED -ge 21 ]]; then
    record PASS "Localization tests (passed=$LOC_PASSED ≥ 21 expected)"
else
    record FAIL "Localization tests (rc=$LOC_RC passed=$LOC_PASSED expected ≥ 21)"
fi

# ── 2c. Confirm RecentScans coverage still green (P10.6f regression) ─
echo "[step] re-confirm RecentScans coverage (P10.6f regression guard)"
...
if [[ $RS_RC -eq 0 && $RS_PASSED -ge 16 ]]; then
    record PASS "RecentScans tests still green (passed=$RS_PASSED ≥ 16)"
else
    record FAIL "RecentScans tests regression (rc=$RS_RC passed=$RS_PASSED)"
fi
