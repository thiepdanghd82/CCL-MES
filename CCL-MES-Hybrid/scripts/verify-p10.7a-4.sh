#!/usr/bin/env bash
# P10.7a-1.4 end-to-end verify script. SUPERSET of verify-p10.7a-3.sh
# + closes the test belt for the entire 7a-1 stack:
#   - 144-cell transition matrix xUnit (replaces the partial subset
#     shipped with 7a-1.1).
#   - N=50 concurrency soak (contract §10.6.3 threshold; 7a-1.3
#     ships N=10 sanity).
#   - audit-state-machine-emits.sh CI grep (every WO-mutating file
#     has paired emit + RV gate).
#   - bUnit render tests against the REAL <WorkOrders /> Razor page.
#   - SAME wire probes from 7a-1.3 (kept verbatim for continuity).
#
...
AUDIT_GREP_SCRIPT="$HYBRID_ROOT/scripts/audit-state-machine-emits.sh"
...
# P10.7a-1.4 — bUnit render tests against the real <WorkOrders />
# Razor page. Closes the gap that today's checkpoint exposed: the
# orchestrator unit tests passed but the operator had no visual
# feedback because the page didn't bind the success banner.
echo "[step] full CCL.MES.Hybrid.Razor.Tests (bUnit render — WorkOrders page)"
RL="$(mktemp)"; dotnet test "$RAZOR_TESTS" --nologo --verbosity quiet > "$RL" 2>&1
P=$(grep -oE "Passed:\s*[0-9]+" "$RL" | head -1 | grep -oE "[0-9]+" | tail -1)
F=$(grep -oE "Failed:\s*[0-9]+" "$RL" | head -1 | grep -oE "[0-9]+" | tail -1)
if [[ "$F" == "0" && -n "$P" && "$P" != "0" ]]; then
    record PASS "Hybrid.Razor.Tests ($P PASS / 0 FAIL — bUnit render)"
else
    tail -10 "$RL"
    record FAIL "Razor.Tests ($P/$F)"
fi

# P10.7a-1.4 — 144-cell transition matrix probe.
echo "[step] full 144-cell transition matrix (legacy CCL.MES.Tests)"
MX="$(mktemp)"; dotnet test "$LEGACY_TESTS" --filter "FullyQualifiedName~FullMatrix" --nologo --verbosity quiet > "$MX" 2>&1
P=$(grep -oE "Passed:\s*[0-9]+" "$MX" | head -1 | grep -oE "[0-9]+" | tail -1)
F=$(grep -oE "Failed:\s*[0-9]+" "$MX" | head -1 | grep -oE "[0-9]+" | tail -1)
if [[ "$F" == "0" && "$P" == "146" ]]; then
    record PASS "144-cell matrix (146 PASS — 144 cells + 2 meta)"
else
    record FAIL "Matrix ($P/$F — expected 146)"
fi

# P10.7a-1.4 — N=50 concurrency soak.
echo "[step] N=50 concurrency soak"
S50="$(mktemp)"; dotnet test "$API_TESTS" --filter "FullyQualifiedName~Concurrent_advances_at_N_equals_50" --nologo --verbosity quiet > "$S50" 2>&1
P=$(grep -oE "Passed:\s*[0-9]+" "$S50" | head -1 | grep -oE "[0-9]+" | tail -1)
F=$(grep -oE "Failed:\s*[0-9]+" "$S50" | head -1 | grep -oE "[0-9]+" | tail -1)
if [[ "$F" == "0" && "$P" == "1" ]]; then
    record PASS "Concurrency soak N=50 (1 winner + 49 conflicts)"
else
    record FAIL "Soak N=50 ($P/$F)"
fi

# P10.7a-1.4 — CI grep audit.
echo "[step] audit-state-machine-emits CI grep"
if bash "$AUDIT_GREP_SCRIPT" > /tmp/audit-grep.log 2>&1; then
    record PASS "CI grep audit (every WO-mutating file has paired emit + RV gate)"
else
    tail -10 /tmp/audit-grep.log
    record FAIL "CI grep audit — see /tmp/audit-grep.log"
fi
...
for pair in \
    ...
    "WoCodeNormalizer:manual_entry_normalisation" \
    "WorkOrdersPageTests:bUnit_render (banner + manual entry + double-tap guard)"; do
    filter="${pair%%:*}"
    label="${pair##*:}"
    FL="$(mktemp)"
    if [[ "$filter" == "WorkOrdersPageTests" ]]; then
        dotnet test "$RAZOR_TESTS" --filter "FullyQualifiedName~$filter" --nologo --verbosity quiet > "$FL" 2>&1
    else
