...
///
/// Guard rails (P10.6a hotfix — Henry's recurring "renderer dead"
/// incident):
...
///   - The OUTER loop is ALSO wrapped so a throw from
///     <c>WaitForNextTickAsync</c>, <c>PeriodicTimer.Dispose</c>, or
///     any DI resolution failure CANNOT escape this
///     <see cref="BackgroundService"/>. An unobserved
///     <see cref="Task"/> exception here used to be caught by
///     <see cref="GlobalErrorLogger"/> only after the renderer had
///     already wound down. The double-wrap is observation-cheap and
...
internal sealed class DeviceHeartbeatHostedService : BackgroundService
{
    ...
    {
        // P10.6a outer guard — even a throw escaping the inner loop
    ...
    /// <summary>Wrap <see cref="PeriodicTimer.WaitForNextTickAsync"/>
    /// so a transient throw (dotnet#71860) is contained.</summary>
